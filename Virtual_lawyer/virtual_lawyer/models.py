...
import chatbot.general_utils
import chatbot.chat_command_handler as chat_command_handler
from chatbot.chat_settings import ChatSettings
...
class Chatbot(models.Model):
    bot = None
    settings = None
    model_dir, hparams, checkpoint = chatbot.general_utils.initialize_session_server()
    @classmethod
    def get_ChatBot(self):
        ...
        if self.bot is None:
            ...
            with ChatbotModel(mode = "infer",
                ...
                self.bot = model
        return self.bot, self.settings

    @classmethod
    def get_bot_response(self, question, model, chat_settings):
        print ("Entered in bot response")
        chatlog_filepath = path.join(self.model_dir, "chat_logs", "web_chatlog_{0}.txt".format(datetime.today().strftime("%Y%m%d")))
...
class ChatbotModel(object):
    ...
    def get_response(self, User,myBot, mySettings):
        print("Entered in model get response")
        response = Chatbot.get_bot_response(User, myBot, mySettings)
        return response
...
Fields_CHOICES = ((1, 'Admiralty (Maritime) Law Cases'), (2, 'Bankruptcy Law'),
                     (3, 'Business (Corporate) Law'), (4, 'Civil Rights Law'), 
                     (5, 'Criminal Law'), (5, 'Entertainment Law'), (5, 'Environmental Law'), (5, 'Real Estate Law'), (5, 'Family Law'), (5, 'Tax Law'))
class Lawyer(models.Model):

    Name = models.CharField(max_length=40)
    Address = models.CharField("Office Address",max_length=50)
    Contact = models.CharField("Contact Number",max_length=20)
    Expertise = models.CharField(max_length=30)
    Chamber = models.CharField("Chamber Number",max_length=20)
    minFee=models.IntegerField("Minimm Fee")
    maxFee=models.IntegerField("Maximum Fee ")
    officeHours=models.CharField("Office Hours",max_length=60)
